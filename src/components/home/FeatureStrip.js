        ...
        <ul className="grid w-full grid-cols-2 lg:h-full lg:grid-cols-4 lg:items-stretch">
          {items.map((item, index) => (
            ...
            >
              ...
              <h3 className="flex h-full w-full items-start justify-center text-[0.68rem] font-semibold uppercase leading-relaxed tracking-[0.18em] text-ink md:mt-3.5 md:h-auto md:items-center lg:mt-0 lg:h-[42px] lg:leading-snug">
                {item.title}
              </h3>
              <p className="w-full max-w-[12.5rem] text-[0.8rem] leading-6 text-ink-soft md:mt-2.5 lg:mt-0 lg:text-[13px] lg:leading-[1.45]">
