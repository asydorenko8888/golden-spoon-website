          ...
          {items.map((item) => (
            <li key={item.number} className="min-w-0">
              ...
              <div className="mt-3 flex gap-3 lg:mt-3.5">
                <p className="shrink-0 font-serif text-[1.15rem] leading-none text-gold">
                  <span className="md:hidden">{Number(item.number)}</span>
                  <span className="hidden md:inline">{item.number}</span>
                </p>
                <div className="min-w-0">
                  <h2 className="font-serif text-[1rem] leading-[1.15] font-medium tracking-tight text-ink uppercase lg:text-[1.05rem]">
                    {item.title.map((line) => (
                  ...
                  <p className="mt-1.5 text-[0.78rem] leading-[1.45] text-ink-soft lg:text-[13px] lg:leading-[1.5]">
                    {item.copy}
                  </p>
                </div>
              </div>
