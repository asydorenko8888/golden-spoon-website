      ...
      <Container className="py-0 md:py-9 lg:flex lg:h-full lg:items-center lg:py-0">
        <ul className="grid w-full grid-cols-2 lg:h-full lg:grid-cols-4 lg:items-stretch">
          {items.map((item, index) => (
            <li
              key={item.title}
              className={`grid min-h-0 grid-rows-[auto_2.25rem_auto] content-start items-start justify-items-center gap-y-2.5 px-3 py-3.5 text-center sm:px-6 md:flex md:grid-rows-none md:flex-col md:items-center md:gap-y-0 md:py-6 lg:h-full lg:justify-center lg:px-7 lg:py-0 ${
