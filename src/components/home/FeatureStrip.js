...
export default function FeatureStrip({ items }) {
  return (
    <section
      ...
      className="border-y border-ink/10 bg-ivory-warm lg:box-border lg:min-h-[var(--feature-height)]"
    >
      <Container className="py-0 md:py-11 lg:flex lg:min-h-[var(--feature-height)] lg:items-stretch lg:py-0">
        <h2 className="sr-only">Why Golden Spoon</h2>
        <ul className="grid w-full grid-cols-2 lg:min-h-[var(--feature-height)] lg:grid-cols-4 lg:items-stretch">
          {items.map((item, index) => (
            <li
              key={item.icon}
              className={`grid min-h-0 grid-rows-[auto_2.25rem_auto] content-start items-start justify-items-center gap-y-2.5 px-3 py-4 text-center sm:px-5 md:flex md:grid-rows-none md:flex-col md:items-center md:gap-y-0 md:py-6 lg:grid lg:h-auto lg:grid-rows-[40px_2.25rem_auto] lg:items-start lg:justify-items-center lg:gap-y-2 lg:px-7 lg:py-3 ${
            ...
            >
              ...
              <h3 className="flex h-full w-full items-start justify-center text-[0.68rem] font-semibold uppercase leading-relaxed tracking-[0.18em] text-ink lg:text-[0.816rem] md:mt-3.5 md:h-auto md:items-center lg:mt-0 lg:h-[2.25rem] lg:items-start lg:leading-snug">
                {item.title}
              </h3>
