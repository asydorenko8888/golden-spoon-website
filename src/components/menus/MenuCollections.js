...
function MenuBlock({ item, imageLeft }) {
  return (
    <div className="lg:grid lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)] lg:items-center lg:gap-x-[8%]">
      ...
      >
        <p className="font-serif text-[1.15rem] leading-none text-gold">
          <span className="md:hidden">{Number(item.number)}</span>
          <span className="hidden md:inline">{item.number}</span>
        </p>
        <h3 className="mt-2 font-serif text-[1.7rem] leading-[1.08] font-medium tracking-tight text-ink uppercase sm:text-[1.95rem] lg:text-[2.15rem]">
          {item.title}
        </h3>
        ...
        {item.note ? (
          <p className="mt-3.5 text-[0.95rem] leading-7 text-ink-soft md:mt-4 lg:text-[15px] lg:leading-[1.7]">
            {item.note}
          </p>
        ) : (
