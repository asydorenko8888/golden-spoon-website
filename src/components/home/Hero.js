...
function HeroScript({ text = "" }) {
  const scriptClass =
    "mt-2 font-script text-[1.65rem] leading-[1.15] text-gold sm:text-[1.9rem] md:mt-2.5 lg:mt-2 lg:text-[clamp(25px,2.2vw,34px)] lg:leading-[1.05]";
  const firstLineClass =
    "block text-[1.8975rem] sm:text-[2.185rem] lg:text-[clamp(28.75px,2.53vw,39.1px)]";
...
function HeroCopy({ text = "" }) {
  const copyClass =
    "mt-2.5 text-[0.95rem] leading-6 text-ink md:mt-3.5 lg:mt-2.5 lg:-translate-y-[10px] lg:text-[15px] lg:leading-[1.45]";
      ...
      <div className="relative lg:grid lg:h-full lg:min-h-0 lg:grid-cols-[45%_55%] lg:items-stretch">
        <div className="relative z-[2] flex flex-col items-start px-5 py-8 sm:px-8 md:py-16 lg:h-full lg:items-center lg:justify-center lg:px-8 lg:py-0">
          <div className="w-full max-w-[34rem] lg:text-center">
            <p className="text-[0.7rem] font-medium uppercase tracking-[0.3em] text-ink lg:text-[12px]">
              {content.eyebrow}
            </p>
            ...
            <h1 className="mt-2.5 font-serif text-[2.45rem] leading-[0.98] font-medium tracking-[-0.01em] text-ink uppercase max-[400px]:text-[2.05rem] md:mt-3 md:text-5xl lg:mt-2.5 lg:text-[clamp(44px,3.6vw,56px)] lg:leading-[0.95]">
            ...
            <div className="mt-4 md:mt-6 lg:mt-0 lg:flex lg:justify-center">
              <Button
                ...
                variant="goldOutline"
                className="min-h-[44px] min-w-[17.5rem] px-8 lg:h-[40px] lg:min-h-[40px] lg:w-[10.9rem] lg:min-w-[10.9rem] lg:px-2 lg:tracking-[0.1em] lg:whitespace-nowrap"
              >
                {content.cta.label}
              </Button>
            </div>
